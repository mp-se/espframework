...
static int fline(char **line, size_t *n, FILE *fp) {
  ...
  if (!*line)
    if (!(*line = reinterpret_cast<char *>(malloc((*n = 64))))) return -1;
  chr = *n;
  pos = *line;
  for (;;) {
    int c = fgetc(fp);
    if (chr < 2) {
      *n += (*n > 16) ? *n : 64;
      chr = *n + *line - pos;
      if (!(*line = reinterpret_cast<char *>(realloc(*line, *n)))) return -1;
      pos = *n - chr + *line;
    }
...
#endif
    if ((fp = fopen(buffer, mode))) return fp;
  }
...
#define s(IDENT) styled(style, IDENT)
  ...
  usage:
    ...
    fprintf(stderr,
            "   -p         - specify a prefix for symbol names (default is "
            "\"g\")\n");
    fprintf(stderr,
            "   -S<style>  - specify a style for symbol generation (default is "
            "\"camelcase\")\n");
    fprintf(stderr,
  ...
  for (i = 0, paths = 0; i < argc; i++) {
    if (!strcmp(argv[i], "-o")) {
      if (i + 1 < argc) {
        snprintf(outfile, sizeof(outfile), argv[i + 1]);
      ...
      snprintf(prefix, sizeof(prefix), argv[i + 1]);
      memmove(argv + i + 1, argv + i + 2, (argc - i - 2) * sizeof *argv);
      argc--;
      continue;
      ...
      snprintf(search_paths[paths], sizeof(search_paths[paths]), name);
      paths++;
      continue;
      ...
      snprintf(file_paths[files], sizeof(file_paths[files]), argv[i]);
      files++;
    }
  }
  ...
  if (strcmp(prefix, "g")) fprintf(out, "#define INCBIN_PREFIX %s\n", prefix);
  if (style != 0) fprintf(out, "#define INCBIN_STYLE INCBIN_STYLE_SNAKE\n");
